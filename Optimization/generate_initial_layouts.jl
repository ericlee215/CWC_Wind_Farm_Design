...
function generate_random_layout_polygon_boundary(layout_filename, nturbines, rotor_diameter, boundary_vertices, boundary_normals; layout_directory="", min_spacing=7.0, layout_number=0)

    # ----- SET UP -----
    # set bounding box
    xmin = minimum(boundary_vertices[:,1])
    xmax = maximum(boundary_vertices[:,1])
    ymin = minimum(boundary_vertices[:,2])
    ymax = maximum(boundary_vertices[:,2])
    xrange = [xmin; xmax]
    yrange = [ymin; ymax]
    ...
    for i = 1:nturbines
        ...
        while !good_point && count < 1e6
            ...
            count += 1
            # generate random point in the bounding box
            locations[i,:] = (rand(1,2) .- 0.5).*[xrange[2]-xrange[1] yrange[2]-yrange[1]] .+ [sum(xrange)/2 sum(yrange)/2]
            # calculate the distance(s) from the point to the boundary(s)
            distances_to_boundaries = ff.ray_trace_boundary(boundary_vertices, boundary_normals, locations[1:i,1], locations[1:i,2])
            # determine if the point is inside the wind farm boundary (+ distance is outside, - distance is inside)
            for j = 1:length(distances_to_boundaries)
                if distances_to_boundaries[j] > 0.0
                    good_point = false
...
function plot_initial_layout_polygon(figure_filename, layout_filename, rotor_diameter, boundary_vertices, boundary_normals; figure_directory="", layout_directory="", save_fig=true, show_fig=false)
    ...
    # set figure axes
    axis("square")
    xlim(xmin - (xmax-xmin)*0.1, xmax + (xmax-xmin)*0.1)
...
"""
    _calc_boundary_normals(boundary_vertices::Array{Float64,2})
Internal function that calculates the unit vectors perpendicular to each boundary face.
# Arguments
- `boundary_vertices::Array{Float64,2}` : n-by-2 array containing all the boundary vertices, counterclockwise
"""
function _calc_boundary_normals(boundary_vertices::Array{Float64,2})
...
boundary_normals = _calc_boundary_normals(boundary_vertices)
...
start_layout_number = 311
end_layout_number = 400
...
if generate_layout_file
    ...
    for layout_number = layout_numbers
        ...
        generate_random_layout_polygon_boundary(layout_filename, nturbines, rotor_diameter, boundary_vertices, boundary_normals; layout_directory=layout_directory, min_spacing=8.0, layout_number=layout_number)
    end
end
...
if generate_layout_figure
    ...
    for layout_number = layout_numbers
        ...
        plot_initial_layout_polygon(figure_filename, layout_filename, rotor_diameter, boundary_vertices, boundary_normals; figure_directory=figure_directory, layout_directory=layout_directory)
    end
end
